...
def main_fun(mentor3D, mentor_json, mentor_video, convalescent3D, convalescent_video, folder_name, output_data):

    # Reads the mentor's json file
    with open(mentor_json, 'r') as file:
        data = json.load(file)
    # Retrieve the anchor frame's information.
    mentor_HightlightFrame = [item['frameCount'] for item in data]

    # Load video
    mentorVideo = np.load(mentor3D)
    ConvalescentVideo = np.load(convalescent3D)

    # Determine video title based on convalescent type
    if "Left" in convalescent_video:
        video_title = "Left"
    elif "Right" in convalescent_video:
        video_title = "Right"
    else:
        video_title = "Front"
    ...
    # Store the output text in json file
    output_data.append({
        "video_title": video_title,
        "mentor_anchor_frames": mentor_HightlightFrame,
    ...
    output_data.append({
        "message": f"The 'Visualize TALMA Matching Behavior' chart was successfully generated and saved as '{folder_name}/TALMA P3Matching(Front-{video_title}).png'."
    })
    ...
    save_pair_pic(mentor_video, mentor_HightlightFrame, convalescent_video, result, f"{folder_name}/Front-{video_title}", similarity_full)
    output_data.append({
        "message": f"The 'Matching Picture Pairs' were successfully generated and saved in the '{folder_name}/Front-{video_title}/' folder."
    })
...
if __name__ == '__main__':
    ...
    convalescent_dict = config['convalescent']
    ...
    # Run for each convalescent type
    for position, convalescent in convalescent_dict.items():
        main_fun(
            mentor['3DModel'],
            mentor['annotate'],
            mentor['video'],
            convalescent['3DModel'],
            convalescent['video'],
            folder_name,
            output_data
        )

    # Save all outputs to output.json
    with open('output.json', 'w') as file:
        json.dump(output_data, file, indent=4)
    print(f"output text have been saved to output.json.")